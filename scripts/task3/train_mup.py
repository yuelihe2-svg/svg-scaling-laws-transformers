"""
Train μP decoder-only LM (Part 3). Uses `mup.set_base_shapes` + `MuAdamW`.
Learning-rate schedule must **scale each param-group lr relative to its μP initial lr**
(see `scripts/task3/README.md`).

  python -m scripts.task3.train_mup --train-jsonl ... --val-jsonl ... --spm-model ... \\
    --preset tiny --lr 3e-4 --out-dir outputs/task3/tiny
"""
...
from scripts.task3.config_presets import MU_PRESETS
from scripts.task3.model_mup import apply_mup_base_shapes, build_model_for_preset
...
def main() -> int:
    ...
    p.add_argument("--preset", type=str, choices=list(MU_PRESETS.keys()), default="tiny")
    p.add_argument("--base-preset", type=str, choices=list(MU_PRESETS.keys()), default="tiny")
    p.add_argument("--delta-preset", type=str, choices=list(MU_PRESETS.keys()), default="small")
    p.add_argument("--block-size", type=int, default=512)
    p.add_argument("--tokens-per-batch", type=int, default=32768)
    p.add_argument("--lr", type=float, default=3e-4, help="Peak LR (same semantics as Part 2 sweep).")
    ...
    p.add_argument("--out-dir", type=Path, default=REPO_ROOT / "outputs" / "task3" / "run")
    ...
    preset = MU_PRESETS[args.preset]
    base_p = MU_PRESETS[args.base_preset]
    delta_p = MU_PRESETS[args.delta_preset]
    ...
    model = build_model_for_preset(
        preset, vocab_size=vocab_size, max_seq_len=block_size, dropout=args.dropout
    ).to(device)
    apply_mup_base_shapes(model, base_preset=base_p, delta_preset=delta_p, dropout=args.dropout)
    model.init_weights_post_mup()

    n_params = model.count_parameters()
    print(f"μP preset={preset.name} | trainable params: {n_params:,}")
    ...
    opt = MuAdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, 0.95))
        ...
        json.dumps(
            {
                ...
                "base_preset": base_p.name,
                "delta_preset": delta_p.name,
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    while step < max_steps:
        ...
        scale = lr_scalar / peak_lr if peak_lr > 0 else 0.0
        for pg in opt.param_groups:
        ...
        if step % 50 == 0 or step == max_steps - 1:
            print(
                f"step {step}/{max_steps} train_loss={loss.item():.4f} lr={lr_scalar:.2e} tok/s={tok_s:.0f}",
                flush=True,
            )
