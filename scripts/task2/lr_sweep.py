"""
Run several learning rates on the Tiny preset (1 epoch each) and save val_loss CSV.

  python -m scripts.task2.lr_sweep --train-jsonl ... --val-jsonl ... --spm-model ... \\
    --lrs 1e-4 3e-4 1e-3 3e-3 1e-2 --out-csv outputs/task2/lr_sweep.csv

Then pick best `--lr` for all other presets.
"""
...
def main() -> int:
    ...
    p.add_argument("--out-csv", type=Path, default=REPO_ROOT / "outputs" / "task2" / "lr_sweep.csv")
    args = p.parse_args()

    args.out_csv.parent.mkdir(parents=True, exist_ok=True)
    rows: list[dict] = []
    for lr in args.lrs:
        out = args.out_csv.parent / f"sweep_tiny_lr{lr}"
        cmd = [
            sys.executable,
            "-m",
            "scripts.task2.train",
